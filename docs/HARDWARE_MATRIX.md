...
- `[runtime].worker_threads` — from the CPU logical core count.
  `blocking_threads` is not core-derived: its tasks park rather than
  compute, so it keeps the sized default (64).
